...
void drawPlainTrajectory(Point3f realPoint)
{
    ...
    Point QRCentre((plainSize.width-QRSideLength)/scale,(plainSize.height-QRSideLength)/scale);

    Mat plain (plainSize, CV_8UC3, Scalar(200,255,255));
    namedWindow("Plain",WINDOW_AUTOSIZE); 

    rectangle(plain, Rect(QRCentre, Point(QRCentre.x+QRSideLength/scale,QRCentre.y+QRSideLength/scale)), Scalar(0,0,0),2);
    line(plain, Point(margin.x, plainSize.height-margin.y), Point(margin.x + QRSideLength/scale, plainSize.height-margin.y), Scalar(0,0,0), 2);
    line(plain, Point(margin.x + QRSideLength/scale, plainSize.height-margin.y), Point(10 + QRSideLength/scale, plainSize.height-margin.y-5), Scalar(0,0,0), 2);
    putText(plain, "105 mm", Point(5 + QRSideLength/scale, plainSize.height-margin.y-10), FONT_HERSHEY_PLAIN , 1, Scalar(0,0,0), 1);

    line(plain,QRCentre,Point(QRCentre.x+QRSideLength/scale,QRCentre.y),Scalar(0,255,0),2);
    putText(plain, "X", Point(QRCentre.x+QRSideLength/scale-10,QRCentre.y-10), FONT_HERSHEY_PLAIN , 1, Scalar(0,0,0), 1);
    line(plain,QRCentre,Point(QRCentre.x,QRCentre.y+QRSideLength/scale),Scalar(0,0,255),2);
    putText(plain, "Y", Point(QRCentre.x-20,QRCentre.y+QRSideLength/scale), FONT_HERSHEY_PLAIN , 1, Scalar(0,0,0), 1);
    circle(plain, QRCentre,5,Scalar(255,0,0),FILLED);   
    ...
    if (trajectory_points.size() >= 2)
        for (size_t i = 2; i < trajectory_points.size()-1; i++)
            line(plain, trajectory_points[i-1], trajectory_points[i],Scalar(255,0,255), 2);

    circle(plain, trajectory_points.back(), 4, Scalar(0, 255, 0), FILLED);
    stringstream ss;
    ss << "(" << realPoint.y << "; " << realPoint.x << ")";
    putText(plain,ss.str(),Point(trajectory_points.back().x+5,trajectory_points.back().y-5),FONT_HERSHEY_PLAIN, 1, Scalar(0,0,0), 1);
